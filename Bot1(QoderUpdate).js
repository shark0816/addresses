export default {
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil(
      updateAndCommit(env).catch(err => {
        console.error("❌ 定时任务异常:", err.message, err.stack);
      })
    );
  }
};
...
const MIN_LINES_THRESHOLD = 5;     // 最终结果低于此数量则告警跳过

// 数据源配置（增删只需改这里）
const BESTCF_SOURCES = [
...
const API_SOURCES = {
  vps789Top: "https://vps789.com/openApi/cfIpTop20",
  vps789Dyn: "https://vps789.com/openApi/cfIpApi",
...
function parseAddressToken(raw) {
  ...
  if (ipv6) {
    const host = `[${ipv6[1]}]`;
    const port = ipv6[2] ? parseInt(ipv6[2], 10) : 443;
    ...
    if (putRes.status === 409) {
      ...
      if (r2.ok) {
        ...
        if (sha2) {
          ...
          console.error("❌ 重试获取 SHA 失败，放弃本次更新");
          return new Response("⚠️ SHA 冲突且重试失败，跳过更新", { status: 200 });
        }
      } else {
        console.error("❌ 重试 GET 文件失败:", r2.status, r2.error);
        return new Response("⚠️ SHA 冲突且重试失败，跳过更新", { status: 200 });
      }
    }
...
async function updateIPs() {
  ...
  const [topRes, dynRes, uo, ...bestcfResults] = await Promise.all([
    fetchWithTimeout(API_SOURCES.vps789Top, { headers: baseHeaders }),
    fetchWithTimeout(API_SOURCES.vps789Dyn, { headers: baseHeaders }),
    fetchWithTimeout(API_SOURCES.uouin, { headers: baseHeaders, redirect: "follow" }),
    ...BESTCF_SOURCES.map(src => fetchWithTimeout(src.url, { headers: baseHeaders }))
  ]);
  ...
  if (lines.length < MIN_LINES_THRESHOLD) {
    console.warn(`⚠️ 最终仅 ${lines.length} 条，低于阈值 ${MIN_LINES_THRESHOLD}，可能数据源异常，跳过提交`);
    return "";
  }
